...
namespace Valve.VR.InteractionSystem
{
    ...
    {
        //剣の演出用BeamManager取得
        GameObject beamManager;
        BeamManager beamManagerScript;
        //ビーム出す用のフラグ
        public bool sword_Right;
        public bool sword_Left;
        ...
        protected virtual void Start()
        {
            //剣の演出用BeamManager初期化
            isGrabing = false;
            beamManager = GameObject.Find("BeamManager");
            beamManagerScript = beamManager.GetComponent<BeamManager>();
        ...
        protected virtual void OnAttachedToHand(Hand hand)
        {
            ...
            //剣を握ったらビーム演出をOnにする
            if (this.gameObject.name == "EnergySwordS01_M1_Title")
            {
                beamManagerScript.BeamOn();
            }
            if (this.gameObject.name == "EnergySwordS01_M1_Left")
            {
                sword_Left = true;
                beamManagerScript.LeftBeamOn();
            }
            if (this.gameObject.name == "EnergySwordS01_M1_Right")
            {
                sword_Right = true;
                beamManagerScript.RightBeamOn();
            }
        ...
        public virtual void OnDetachedFromHand(Hand hand)
        {
            ...
            if (this.gameObject.name == "EnergySwordS01_M1_Title")
            {
                beamManagerScript.BeamOff();
            }
            if (this.gameObject.name == "EnergySwordS01_M1_Left")
            {
                beamManagerScript.LeftBeamOff();
            }
            if (this.gameObject.name == "EnergySwordS01_M1_Right")
            {
                beamManagerScript.RightBeamOff();
            }
